...
func builtinSubstr(i *interpreter, trace traceElement, inputStr, inputFrom, inputLen value) (value, error) {
	strV, ok := inputStr.(*valueString)
	if !ok {
		msg := fmt.Sprintf("substr first parameter should be a string, got %T", inputStr)
		return nil, makeRuntimeError(msg, i.getCurrentStackTrace(trace))
	}

	fromV, ok := inputFrom.(*valueNumber)
	if !ok {
		msg := fmt.Sprintf("substr second parameter should be a number, got %T", inputFrom)
		return nil, makeRuntimeError(msg, i.getCurrentStackTrace(trace))
	}
	...
	lenV, ok := inputLen.(*valueNumber)
	if !ok {
		msg := fmt.Sprintf("substr third parameter should be a number, got %T", inputLen)
		return nil, makeRuntimeError(msg, i.getCurrentStackTrace(trace))
	}

	if math.Mod(lenV.value, 1) != 0 {
		msg := fmt.Sprintf("substr third parameter should be an integer, got %f", inputLen)
		return nil, makeRuntimeError(msg, i.getCurrentStackTrace(trace))
	}

	lenInt := int(lenV.value)
